...
export default function FocusTimer({ onBackToLobby }) {
  ...
  return (
    <div className="w-full p-6 text-slate-100 max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold tracking-wider flex items-center gap-2 text-slate-100">
              <Clock className="w-5 h-5 text-sky-300" /> FOCUS TIMER
            </h1>
          </div>
        </div>
        <div className="flex items-center gap-2 font-mono text-xs bg-amber-400/10 text-amber-300 px-3 py-1.5 rounded-full border border-amber-400/30">
      ...
      <div className="glass-panel p-8 text-center space-y-8 relative overflow-hidden">
          ...
          {PRESETS.map((p) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-xs font-semibold border transition-all ${
                activePreset === p.id 
                  ? `${p.color} scale-105 shadow-md border-white/40` 
                  : 'bg-slate-900/30 border-white/10 text-slate-400 hover:bg-slate-800/40'
              }`}
            >
              {p.label} ({p.minutes}m)
            </button>
        ...
        <div className="py-6">
          <div className="text-7xl sm:text-8xl font-mono font-bold tracking-tight text-slate-100 drop-shadow-xl">
            {formatTime(timeLeft)}
          </div>
          <p className="text-xs text-slate-400 uppercase tracking-widest mt-4 flex items-center justify-center gap-2 font-semibold">
        ...
        <div className="flex justify-center items-center gap-4">
          <button
            onClick={toggleTimer}
            className={`glass-button px-8 py-3.5 text-base font-bold flex items-center gap-2.5 border ${
              isRunning ? 'border-rose-400/50 text-rose-300 hover:bg-rose-500/20' : 'border-emerald-400/50 text-emerald-300 hover:bg-emerald-500/20'
            }`}
          >
            {isRunning ? <><Pause className="w-5 h-5" /> Pause</> : <><Play className="w-5 h-5 fill-current" /> Start Focus</>}
          </button>
          <button
            onClick={resetTimer}
            className="glass-button p-3.5 text-slate-400 hover:text-slate-100"
            title="Reset Timer"
          >
